...
5. **Units are stated for every field.** SI everywhere, with one deliberate exception: `CellInstance` in `render_view_v1.h` is in micrometres, because fp32 metres would lose the sub-micron structure the optics model depends on.
...
| `render_view_v1.h` | view modes, overlays, `CellInstance` GL layout, scope state | `docs/RENDERING.md` |
...
- **`render_view_v1.h` `CellInstance`.** Its layout is a GL vertex-attribute contract, asserted at 32 bytes. Changing it means changing `render/cells_pass.cpp` bindings in the same commit.
