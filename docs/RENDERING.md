...
**Implemented at M7b (ADR-029), and the divergence is a golden.** All five modes now draw distinctly, and the two IR modes are the **novel-vs-film split shipped both ways**. Petrovascope is the novel's *emission* instrument: it glows by `emit_power`, and a non-emitting cell is invisible. Thermal IR is the 2026 film's *absorption* image: the warm medium is false-coloured pink/red and the albedo-0 cells are **black silhouettes** on it, with a hot rim on the awake heat-sources (the `AWAKE` latch is exact for that, no `temp_cell` needed). The `m7b_thermal_awake` vs `m7b_petrova_awake` must-differ pair pins that the same idle cell is a visible absorber in one and invisible in the other. The **cross-fade slider lands at M12f**: each mode's cell appearance and its background is a function of `ViewMode`, evaluated for the primary mode and `mode_blend_to` and dissolved in premultiplied alpha (so a cell invisible in one mode adds no colour), with blend 0 bit-identical to the primary mode so no golden moves. The **pre-ignition warm-up lands at M12g** (render_view_v3, ADR-043): a heated dormant cell glows continuously in Thermal IR by its per-cell `temp_c` as it warms toward the setpoint, then holds the hot rim once awake. **Bloom over the Petrova emission lands at M12h** (ADR-044, from a separate emission buffer -- see Sec 5). The **real T-field false-colour behind Thermal IR lands at M12i** (ADR-045): the diffused temperature field is uploaded to a texture and sampled through a warm IR LUT, camera-mapped into the chamber, so a heated plume reads as a bright region and the medium's structure is visible. The flat warm clear it replaces is now the `--no-field` fallback. The render remainder is complete; only packaging (`v1.0`) remains.
