...
- **Motion:** persistent random walk at `TAU_CRAWL_SPEED`, biased up the local cell-density gradient (same run-and-tumble as §8, sensing hash-cell occupancy). Same OU integrator with its own γ. **The predator is a water-density blob, so `TAU_MASS` (`ρ_water · TAU_VOLUME`) sets the drag and inertia only** — it is neutrally buoyant, no sedimentation.
- **Predation:** on overlap with a live cell, engulf. Cell → dead. The Taumoeba digests for `TAU_DIGEST_TIME`, then adds `prey_biomass · TAU_BIOMASS_YIELD` to its **dry biomass**. Per Weir's own resolution, it consumes only the **chemical** energy — the neutrino store is handled by the §10 toggle, and `void` is the canon-consistent default.
- **Nitrogen lethality:** `hazard = max(0, N_local − TAU_N2_LETHAL_CONC · (1 + tolerance · TAU_N2_TOLERANCE_K))`, and death with probability `1 − exp(−hazard · TAU_N2_HAZARD_RATE · dt_bio)` per tick. `TAU_N2_TOLERANCE_K` sets how much full tolerance buys (=1: a `tol`=1 strain survives twice the base lethal concentration); `TAU_N2_HAZARD_RATE` is *derived* from a survival time `TAU_N2_LETHAL_TIME`, not guessed (ADR-030). `N_local` is a bilinear field read, in kg/m³ — unlike engulfment, the grid is the right instrument. Every alive Taumoeba draws **one uniform unconditionally** from its own stream (survivor and dier alike), so the stream stays a pure function of state (ADR-022).
- **Evolution:** the Taumoeba's `biomass` field is its **dry biomass** (initial value `TAU_MASS_DRY`, the growth variable), distinct from the water-blob mass `TAU_MASS` that sets its drag — a mostly-water amoeba, exactly as a cell's `CELL_MASS_DRY` is distinct from its total mass (ADR-030). It **divides at 2× `TAU_MASS_DRY`** (`TAU_DIVIDE_BIOMASS`), faithful to "2× initial biomass" and reachable in ~8 prey; biomass splits in half, daughter slots come from a prefix sum (never atomicAdd), and the daughter inherits `clamp(tolerance + N(0, TAU_MUTATION_SIGMA), 0, 1)` drawn from the **daughter's** stream (the cell-mitosis rules, ADR-025). A per-Taumoeba `generation` counter records lineage depth. Under a slowly rising nitrogen ramp this reproduces the **Taumoeba-82.5** breeding arc as genuine directional selection rather than as a script (`test_evolution`, ADR-030).
