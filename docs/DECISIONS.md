...
**Context.** M13a wired the field brushes to the mouse. M13b adds the two interactions the milestone reserves: a *light-leash* (drag a light, the culture herds after it) and *optical tweezers* (grab a cell and tow it). Both must stay honest — no faked advection — and both must leave an un-poked run bit-identical (INV-8). Three things had to be settled.

**The light is directional; herding needs a spatial peak, so the leash is a positional spot.** `contract::LightSource` (fields_v1.h) is a *directional plane wave* — a propagation direction and an edge irradiance, attenuated by occlusion to produce P5. Cells phototax by climbing the local irradiance in the Feed state (taxis.cuh), a temporal run-and-tumble. A plane wave is spatially flat, so there is no point to herd toward: dragging its *direction* only leans a dense, self-shadowing culture toward the bright edge, and does nothing to a sparse one. The honest fix is a **focused illumination spot** — a radial irradiance disc (the `(1-t^2)^2` falloff the field brush already uses) peaking at the cursor's chamber point. Awake sub-0.95-charge cells climb its gradient and follow the cursor like a laser pointer, and the herding is the *existing* run-and-tumble — nothing scripted, the same discipline as P1–P5. A focused disc is a real microscope thing (a field stop narrowed to a spot); taken as top-down it casts no in-plane shadow, so P5's directional story is untouched.

**Both are World fields, not a contract bump.** The spot state (`light_spot`, `light_x/y`, `light_spot_radius`, `light_spot_irradiance`) and the trap state (`trap_slot`, `trap_x/y`, `trap_stiffness`) live on `World`, exactly as `ambient_irradiance` and the ADR-035 overrides do — sim-internal interaction state that never crosses the render boundary. Bumping `LightSource` to fields_v2 for a position would cascade to every fields_v1 consumer for something render does not need. The spot is a *separate, additive* path at the top of `emission_step`, gated on `light_spot` (default false): every scenario and every un-poked run takes the directional sweep unchanged, and the scenario's own `light` source is never touched by the tool. Defaults are off/0/-1, so `world_step` is bit-identical to M13a (M0.4 re-run confirms).

**The tweezers are a real optical trap: a harmonic spring, stability-scaled off contact.** `trap_force` (integrator.cuh) adds `F = -k(r - r_target)` to the grabbed cell in the forces kernel (stage 5), in the two in-plane axes the user drags in; z is left to buoyancy, so the trap holds a cell against gravity exactly as a real optical trap does. An explicit overdamped spring is stable only while `k·dt/gamma` stays bounded — the same limit `CONTACT_STIFFNESS = gamma/(8·dt)` is derived to respect (ADR-018) — so the app sets the stiffness as a small multiple of `CONTACT_STIFFNESS` (a HUD `trap_strength`), and the sim scales it by 1/physics_rate like contact (ADR-027). `trap_slot -1` (no grab) never matches a valid index, so the force sum is bit-identical for every cell in an un-grabbed run.

**Consequences / the gate.** Applied at a tick boundary from the app loop, never the input handler (a device write mid-tick breaks INV-8, ADR-040). Live interaction is non-deterministic by design (frame-rate-coupled input), as ADR-040 already established for the brushes; the snapshot does not carry the spot/trap (transient setup state, ADR-036). The input model gains two tools (Light, Grab) appended after the four brushes, so `--auto-poke`'s 1..4 mapping is unchanged. `M13b.1`: `--auto-light` (a stand-in parking a wide spot past the +x wall) herds an awake culture's centroid toward it, headless. `M13b.2`: `--auto-grab slot x y` tows a cell to within a few µm of the target. Both also produce a screenshot; determinism (M0.4) and goldens (M3.2) unmoved. M13b is a parallel arc off m12e-green like M13a — its gate re-runs M0..M12e but not the unbuilt M12f/M12g. Deferred to M13c: recording the interaction alongside the snapshot ring. Deferred within M13b: focal-plane coupling (grab only in-focus cells) — the pick is already 2D and the trap holds z via buoyancy, so it adds little now.
